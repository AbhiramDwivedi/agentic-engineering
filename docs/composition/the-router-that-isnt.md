...
*The router that isn't one: when the input already carries a label, picking its handler is a dictionary lookup, and your code decided the outcome when it wrote the dictionary. LLM routing proper is the newer, narrower pattern: a model classifies unlabelled input before any handler runs.*
...
Two mechanisms in agentic systems answer to the name "routing". The first reads a label that arrived with the input, looks it up in a table your code wrote, and runs the handler it finds there, so nothing is decided while the system is running; the deciding happened when someone typed out the table. The second reads unlabelled free text, classifies it, and only then picks a handler, fresh on every request. One of them is a dictionary. The other pays a model to exercise judgment on every input that comes through the door, and mistaking one for the other costs you in both directions.

Put a classifier in front of inputs that already carry a label and you pay for a model call on every request, with its latency and a fresh chance of a misread, to re-answer a question the input answered on arrival. A lookup table pointed at unlabelled free text fails the other way around: keyword rules, or a category field users will not fill in, send messages to the wrong handler. Neither mistake is obvious when it happens, because the first shows up only as cost spread across every event, while the second produces plausible answers from the wrong specialist, so nothing in the logs looks wrong.
...
It has a place in this chapter anyway, because the two ends of that list are so often confused under the one word, and telling them apart is most of what this chapter is for.

**A dispatch table involves no model and no classifier of any kind.** There is nothing to classify; the caller already supplied the label. This is the sharper form of the deflation 1.2 states in brief: the common objection to calling a lookup "routing" is that no LLM is involved, and the accurate objection is that no classifier of any kind is involved. Your code decides everything, and it decided at authoring time. The technique is ordinary engineering, Standard for decades: a table from key to handler fronts interpreters, event loops, and web frameworks, and the pattern catalogs wrote it up for the web tier more than twenty years ago.[^fowler][^j2ee]
...
Two rows get mistaken for this chapter's patterns most often. Tool use and handoffs both let the model pick, so both read as "routing" in a design review. Neither is a one-time upfront split. A tool choice recurs on every turn of a running exchange, and a handoff fires from inside an already-running agent, transferring the whole conversation to a peer rather than classifying it at the door.[^handoffs] OpenAI's builder guide describes the adjacent manager pattern, a central LLM delegating to specialist agents through tool calls; it is routing-shaped, and its full treatment lives with the multi-agent material in 9.2.[^openai-guide]

## 3. How to do it

Here are both mechanisms in one diagram, in the reference's visual language (rounded nodes are the model deciding, rectangles are your code):
...
Each provider tab below supplies the classify function that `route_message` takes as its first argument. The tabs are asymmetric by design, as in 3.1. The LangGraph tab shows the full graph, classify node, conditional edge, handlers, and escalate node, because the classify-then-branch graph is the reference shape for this pattern.[^langgraph] The two raw-SDK tabs build only `classify_fn`, each vendor's structured-output mechanism filling the `RouteDecision` contract, and hand it to the shared `route_message` above.
...
Walking one run through each mechanism makes the cost difference concrete. The dispatch side's whole trace is two steps:
...
Three issues show up at production scale.
...
Read your input before you pick the mechanism. If a reliable label already exists, use a dictionary: validate the key, give unknown types an explicit deny branch, spend zero tokens, and do not call it agentic. If the input is unlabelled and the categories are stable, few, and latency-sensitive, a non-LLM classifier does the job without a model call per request. If the categories are fuzzy or evolving, spend the model call, and build the failure path before the happy one: keep `unclear` inside the taxonomy, give low confidence a structured escalation to a human, build a labelled eval set for the classifier, and expect to retune as the taxonomy grows.

The two halves fail in opposite directions. A dispatch table built with its deny branch fails loudly, on the first bad key, in the log. A classifier fails quietly, handing a confident answer to the wrong specialist with nothing in the trace to flag it, and you hear about it from the merchant. Instrument the half that decides at runtime.

...
Ask the router at the front of your system what it decides at runtime. One of the two mechanisms that share the name "routing" in agentic systems has an awkward answer: nothing. The label came in with the input, the table was written months ago, and the runtime work is a dictionary lookup. The other mechanism earns the name: it reads unlabelled free text and commits to a judgment on every request. Each mechanism is a mistake in the other's place.

If your inputs already arrive labelled, there is no decision left to make, so a classifier in front of them buys nothing: you pay a model call and its latency on every request, and you accept a fresh chance of a misread, to re-answer a question the input already answered. If your inputs are unlabelled free text, a real decision has to be made on every request, and a lookup table cannot make it: keyword rules, or a category field users will not fill in, send messages to the wrong handler. Both mistakes are hard to spot from outside. The overbuilt door just costs more on every event, and the underbuilt one hands work to the wrong specialist, who still produces a plausible answer, so nothing in the logs looks wrong.

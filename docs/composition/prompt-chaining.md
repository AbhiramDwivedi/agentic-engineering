...
The obvious first version of a listing pipeline is a single prompt. Hand the model the supplier row and the spec sheets and ask for everything at once: a category, a description, bullets, a price, the brand voice. It holds up in a demo, then degrades on a real supplier feed without an error: no exception, no failed request, just answers that get vaguer and less grounded as the context fills. A model holding five jobs in one window does each of them a little worse than a model holding one ([1.5 Context Engineering](../foundations/context-engineering.md) covers the degradation in depth). And an early wrong inference becomes a premise: everything after it builds on the mistake, and no code is positioned to notice.

Take the Aldsworth dual-motor sit-stand desk from Northvale Furnishings, SKU `NV-ALDSWORTH-DM`. Suppose the categorize work inside the mega-prompt goes wrong and the desk lands outside `office/desks/standing-desks`. Nothing fails. The copy gets written for the wrong shelf, the content blocks answer questions the wrong shoppers would ask, the compliance checks run against the wrong category's rules, and the listing publishes into a category where people shopping for standing desks never look. Caught between steps, the bad category costs one extra model call. Caught after publish, it costs a mislisted product, whatever sales the desk missed while it sat on the wrong shelf, and some of the merchandiser's trust in the pipeline.

Prompt chaining is the fix: split the work at the seams where your code can check it. Categorize becomes its own call whose one-field output your code validates against the catalog taxonomy before the copy call ever runs. A failed check re-asks the step once with the validation error folded in; a second failure stops the run loudly, at the step that broke.
